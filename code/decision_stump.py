    ...
    def fit(self, X, y):
        ...
        for d in range(D):
            for n in range(N):
                ...
                y_yes = np.bincount(y[X[:,d] > value])
                n_yes = np.sum(y_yes)

                y_no = np.bincount(y[X[:,d] <= value])
                n_no = np.sum(y_no)

                ## consider putting min_length
                info_gain = entropy(count / np.sum(count)) - (n_yes / N) * entropy(y_yes / np.sum(y_yes))  - (n_no / N) * entropy(y_no / np.sum(y_no))
