...
from simple_decision import SimpleDecisionTree
...
if __name__ == "__main__":
    ...
    if question == "3.4":
        ...
        # print('mode:')
        # print(utils.mode(X))
        # print('quantile .5:', np.percentile(X, 5))
        # print('quantile .25:', np.percentile(X, 25))
        # print('quantile .5:', np.percentile(X, 50))
        # print('quantile .75:', np.percentile(X, 75))
        # print('quantile .95:', np.percentile(X, 95))

        #
        mean_arr = X.mean(axis=0)
        max = mean_arr.max()
        idx = np.argwhere(mean_arr == max)
        print('max mean from:', names[idx])
        min = mean_arr.min()
        idx = np.argwhere(mean_arr == min)
        print('min mean from:', names[idx])

        var_arr = X.var(axis=0)
        max = var_arr.max()
        idx = np.argwhere(var_arr == max)
        print('max var from:', names[idx])
        min = var_arr.min()
        idx = np.argwhere(var_arr == min)
        print('min var from:', names[idx])
        ...
        ##### my CODE
        model = SimpleDecisionTree(max_depth=2, stump_class=DecisionStumpInfoGain)
        model.fit(X, y)
        y_pred = model.predict(X)
        error = np.mean(y_pred != y)
        ...
        fname = os.path.join("..", "figs", "q6_4_simpleDecisionBoundary.pdf")
        plt.savefig(fname)
        print("\nFigure saved as '%s'" % fname)
        ...
        plt.ylabel("y-coordinate")
        plt.legend()
        fname = os.path.join("..", "figs", "q6_5_leastTrainingError.pdf")
        plt.savefig(fname)
        print("\nFigure saved as '%s'" % fname)
